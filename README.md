...
**Red Team** (`/mode redteam`) — Full adversary simulation. Runs the complete kill chain: initial access → execution → persistence → privilege escalation → defense evasion → credential access → discovery → lateral movement → collection → exfiltration. Builds and deploys payloads that bypass AV/EDR — uses obfuscation, memory-resident techniques, and living-off-the-land to evade detection. Establishes persistence at every level gained. Assumes blue team is actively monitoring with SOC, SIEM, and defensive controls in place. OPSEC scoring reflects real-world detection risk. Secret vault and scope enforcement active. Attack planning tracks paths, blockers, and lessons learned across multiple operator sessions. Exit conditions are based on engagement objectives (domain admin, specific data, business email compromise, etc.).
